...
(function () {
  ...
  function updateBadge() {
    try {
      const shippingH3 = [...document.querySelectorAll("h3")].find((h) =>
        h.textContent.includes("Shipping paid"),
      );
      if (!shippingH3) return;

      const shippingPaid = parseDollar(
        shippingH3.nextElementSibling?.textContent,
      );
  ...
  function initObserver() {
    const totalSpan = document.querySelector(
      'div[data-testid="total-cost"] span:nth-child(2)',
    );
    if (!totalSpan) return;
