...
(function() {
    ...
    function parseDollarAmount(text) {
        return parseFloat((text.match(/\$([\d,.]+)/) || [0, 0])[1].replace(/,/g, '')) || 0;
    }

    function updateBadge() {
        try {
            const shippingPaidDiv = [...document.querySelectorAll('h3')]
                .find(h => h.textContent.includes('Shipping paid'));
            const totalCostDiv = document.querySelector('div[data-testid="total-cost"]');

            if (!shippingPaidDiv || !totalCostDiv) return;

            const shippingPaid = parseDollarAmount(shippingPaidDiv.nextElementSibling?.textContent || '');
    ...
    function watchBadge() {
        ...
        if (shippingNode) {
            new MutationObserver(updateBadge).observe(shippingNode, { characterData: true, subtree: true, childList: true });
        }
        if (labelNode) {
            new MutationObserver(updateBadge).observe(labelNode, { characterData: true, subtree: true, childList: true });
        }
    ...
    function initShippingFilter() {
        const shippingFieldset = document.querySelector('fieldset[data-testid="service-list-table"]');
        ...
        function refreshOptions() {
            options = [...shippingFieldset.querySelectorAll('input[type="radio"][name="service"]')];
        }
        ...
        filterBox.addEventListener('keydown', e => {
            ...
            if (e.key === 'ArrowDown') {
                ...
                while (options[selectedIndex].closest('label').style.display === 'none');
                highlightSelected();
    ...
    const interval = setInterval(() => {
        if (document.querySelector('fieldset[data-testid="service-list-table"]') &&
